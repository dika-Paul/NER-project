...
def should_judge(graph_state: GraphState) -> Literal["judge_llm_predict", "decision"]:
...
def should_continue_training(graph_state: GraphState) -> Literal["continue", "end"]:
...
def build_train_graph():
    ...
    graph_builder.add_node("train_ner", train_ner_node)
    graph_builder.add_node("update_model", update_model_node)
    graph_builder.add_node("get_unlabeled_data", get_unlabeled_data_node)
    graph_builder.add_node("model_predict", model_predict_node)
    graph_builder.add_node("bio_to_dict", BIO2dict_node)
    graph_builder.add_node("llm_predict", llm_predict_node)
    graph_builder.add_node("primary_span_to_dict", primary_span2dict_node)
    graph_builder.add_node("compute_ner_llm_distance", labeled_res_distance_compute_node)
    graph_builder.add_node("judge_llm_predict", judge_llm_predict_node)
    graph_builder.add_node("judge_span_to_dict", judge_span2dict_node)
    graph_builder.add_node("compute_judge_distance", judge_res_distance_compute_node)
    graph_builder.add_node("decision", decision_node)
    graph_builder.add_node("add_train_data", add_train_data_node)
    graph_builder.add_node("iteration_update", iteration_update_node)
